package com.fukuni.mobiusgithub.domain.login
...
class LoginUpdate: Update<LoginModel, LoginEvent, LoginEffect> {
    ...
    override fun update(model: LoginModel, event: LoginEvent): Next<LoginModel, LoginEffect> {
        return when(event) {
            ...
            is UserCredentialsLoadedEvent -> Next.next(model.copy(login = event.login, pass = event.pass),
            ...
            is LoginClickEvent -> {
                ...
                return Next.next(model.copy(isLoading = true, error = null),
            ...
            is LoginResponseErrorEvent -> Next.next(model.copy(isLoading = false, error = event.err?.message))
